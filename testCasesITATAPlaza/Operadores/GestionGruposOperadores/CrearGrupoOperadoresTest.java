...
public class CrearGrupoOperadoresTest {
	...
	public void testCrearGrupoOperador1() {
		...
		if (analisis) {
			logger.info("TC1 Crear Grupo Operador: Página Crear grupo operador: análisis sintáctico correcto");
		}
		else {
			logger.error("TC1 Crear Grupo Operador: Página Crear grupo operador: análisis sintáctico incorrecto");
			LoginTest.minors++;
		}
	...
	public void testCrearGrupoOperador2() {
		...
		logger.info("TC2 Crear Grupo Operador: Resultado esperado=true (operación correcta)");
		botest.bo.seleccionarOpcionMenu("Configuración sistema","Operadores","Gestión de grupos");
		GestionGruposOperadoresProcess gestionGrupoOperadores=new GestionGruposOperadoresProcess();
		gestionGrupoOperadores.crear();
		CrearModificarGrupoOperadoresProcess crearGrupoOperador=new CrearModificarGrupoOperadoresProcess();
		String nombreGrupo="Grupo3";
		...
		if (hayError) {
			LoginTest.majors++;
			logger.error("TC2 Crear Grupo Operador: El nombre del nuevo grupo no aparece en el combo al buscar un operador");
		}
		logger.info("TC2 Crear Grupo Operador: Resultado obtenido="+((!hayMsgError) && (!hayError)));
		assertTrue((!hayMsgError) && (!hayError));
	}
	...
	public void testCrearGrupoOperador3() {
		...
		logger.info("TC3 Crear Grupo Operador: resultado esperado=true (aparece un mensaje de error)");
		botest.bo.seleccionarOpcionMenu("Configuración sistema","Operadores","Gestión de grupos");
		GestionGruposOperadoresProcess gestionGrupoOperadores=new GestionGruposOperadoresProcess();
		gestionGrupoOperadores.crear();
		CrearModificarGrupoOperadoresProcess crearGrupoOperador=new CrearModificarGrupoOperadoresProcess();
		crearGrupoOperador.escribirCampoDescripcion("Grupo3");
		Error errorAccesoPlaza=crearGrupoOperador.seleccionarOpcionAccesoPlaza("Cobrador");
		if (errorAccesoPlaza==Error.Error) {
			LoginTest.majors++;
