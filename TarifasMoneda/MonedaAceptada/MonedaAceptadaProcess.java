...
import ventanasITATAHost.ConfiguracionSistema.TarifasMoneda.MonedaAceptada.MonedaAceptadaScreen;
import procesosComunes.FormProcess;
import testsComunes.RutinasTestComunes;

public class MonedaAceptadaProcess extends FormProcess {
	...
	private final TipoBy[] i_botonescrud={TipoBy.ID,TipoBy.ID}; 
	private final String[] str_botonescrud= {"ctl00_ContentZone_BtnCreate","ctl00_ContentZone_BtnDelete"};
	...
	private final static Logger logger = Logger.getLogger(MonedaAceptadaProcess.class);
	 
	public MonedaAceptadaProcess() {
		try {
			screen=new MonedaAceptadaScreen(i_titulo,str_titulo,i_subtitulo,str_subtitulo, i_botones,
	...
	public void crear() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudMonedaAceptada.Crear.ordinal());
	}
	
	public void eliminar() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudMonedaAceptada.Eliminar.ordinal());
	}
	
	public void informe() {
		screen.clickBoton(BotonesMonedaAceptada.Informe.ordinal());
	}
	
	public void enviarAVias() {
		screen.clickBoton(BotonesMonedaAceptada.EnviarAvias.ordinal());
	}
	
	public void volver() {
		screen.clickBoton(BotonesMonedaAceptada.Volver.ordinal());
	}
	...
	public boolean buscarRegistroBDEnTabla (ResultSet rs) {
		...
		try {
			...
			switch (rs.getString("cashtype").trim()) {
				case "C": valores[1]="Moneda"; break;
				case "N": valores[1]="Billete"; break;
				default: break;
			}
			
			DecimalFormat df = new DecimalFormat("#,##0", new DecimalFormatSymbols(new Locale("pt", "BR")));
			Integer value = new Integer(Integer.parseInt(rs.getString("facevalue").trim()));
			valores[2]="$ "+df.format(value.intValue());
			...
			if (resultado!=null) {
				...
				logger.error("No se ha encontrado el elemento ("+valores[0]+","+valores[1]+","+valores[2]+") en la tabla");
				return false;
			}
		}
		catch (Exception e) {
	...
	public boolean buscarRegistroTablaEnBD (ResultSet rs, int numfila) {
		try {
			String[] resultado=obtenerFilaTabla(numfila);
			/*try {
				resultado[2]=String.valueOf(Integer.parseInt(resultado[2].substring(2).replace(".","")));
			}
			catch (Exception e) {
				fail();
			}*/
	...
	public boolean compararRegistroDBFilaTabla(ResultSet rs, String[] resultado) {
		try {
			...
			switch (rs.getString("cashtype").trim()) {
				case "C": row[1]="Moneda"; break;
				case "N": row[1]="Billete"; break;
				default: break;
			}
			DecimalFormat df = new DecimalFormat("#,##0", new DecimalFormatSymbols(new Locale("pt", "BR")));
			Integer value = new Integer(Integer.parseInt(rs.getString("facevalue").trim()));
			row[2]="$ "+df.format(value.intValue());
	...
	public boolean sintacticAnalysis(String titulo, String subtitulo, String[] botones, String[] botonesCRUD,
		...
		logger.debug("Moneda aceptada: inicio análisis sintáctico");
