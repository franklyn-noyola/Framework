...
import ventanasITATAHost.ConfiguracionSistema.ConfiguracionPeaje.Nodos.NodosScreen;
import procesosComunes.FormProcess;
import testsComunes.RutinasTestComunes;

public class NodosProcess extends FormProcess {
	...
	private final TipoBy[] i_botonescrud={TipoBy.ID,TipoBy.ID,TipoBy.ID,TipoBy.ID,TipoBy.ID,TipoBy.ID};
	...
	private final static Logger logger = Logger.getLogger(NodosProcess.class);
	 
	public NodosProcess() {
		try {
			screen=new NodosScreen(i_titulo,str_titulo,i_subtitulo,str_subtitulo, i_botones,
	...
	public void crear() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudNodosPeaje.Crear.ordinal());
	}
	
	public void modificar() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudNodosPeaje.Modificar.ordinal());
	}
	
	public void editarNodoVia() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudNodosPeaje.EditarNodoVia.ordinal());
	}
	
	public void eliminar() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudNodosPeaje.Eliminar.ordinal());
	}
	
	public void activar() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudNodosPeaje.Activar.ordinal());
	}
	
	public void desactivar() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudNodosPeaje.Desactivar.ordinal());
	}
	
	public void informe() {
		screen.clickBoton(BotonesNodos.Informe.ordinal());
	}
	
	public void enviarAVias() {
		((NodosScreen) screen).clickBotonBusqueda(BotonesNodos.EnviarAvias.ordinal());
	}
	
	public void volver() {
		screen.clickBoton(BotonesNodos.Volver.ordinal());
	}
	...
	public boolean buscarRegistroBDEnTabla (ResultSet rs) {
		...
		try {
			valores[0]=rs.getString("plazanode").trim();
	...
	public boolean compararRegistroDBFilaTabla(ResultSet rs, String[] resultado) {
		try {
			...
			row[0]=rs.getString("plazanode").trim();
	...
	public boolean sintacticAnalysis(String titulo, String subtitulo, String[] botones, String[] botonesCRUD,
		...
		logger.debug("Nodos: inicio análisis sintáctico");
