...
import ventanasITATAHost.ConfiguracionSistema.ConfiguracionPeaje.Motivos.MotivosScreen;
import procesosComunes.FormProcess;
import testCasesITATAHost.ConfiguracionSistema.ConfiguracionPeaje.Motivos.MotivosTest;
import testsComunes.RutinasTestComunes;

public class MotivosProcess extends FormProcess {
	...
	private final static Logger logger = Logger.getLogger(MotivosProcess.class);
			
	public MotivosProcess() {
		try {
			screen=new MotivosScreen(i_titulo,str_titulo,i_subtitulo,str_subtitulo, i_botones,
	...
	public void crear() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudMotivos.Crear.ordinal());
	}
	
	public void modificar() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudMotivos.Modificar.ordinal());
	}
	
	public void eliminar() {
		((FormRadioScreen) screen).clickBotonCrud(BotonesCrudMotivos.Eliminar.ordinal());
	}
	
	public void informe() {
		screen.clickBoton(BotonesMotivos.Informe.ordinal());
	}
	
	public void enviarAVias() {
		((MotivosScreen) screen).clickBotonBusqueda(BotonesMotivos.EnviarAvias.ordinal());
	}
	
	public void volver() {
		screen.clickBoton(BotonesMotivos.Volver.ordinal());
	}
	...
	public boolean buscarRegistroBDEnTabla (ResultSet rs) {
		...
		try {
			valores[0]=rs.getString("option").trim();
			String numero=rs.getString("choicetype").trim();
			int indice=0;
			try {
				indice=Integer.parseInt(numero);
			}
			catch (NumberFormatException e1) {
				logger.fatal("El valor del campo choicetype de la tabla AOPTIONCHOICE no es correcto: '"+numero+"'");
				e1.printStackTrace();
				return false;
			}
			valores[1]=MotivosTest.tipos_motivo[indice-1];
	...
	public boolean compararRegistroDBFilaTabla(ResultSet rs, String[] resultado) {
		try {
			...
			row[0]=rs.getString("option").trim();
			int indice=0;
			String numero=rs.getString("choicetype").trim();
			try {
				indice=Integer.parseInt(numero);
			...
			if (indice<1 || indice>MotivosTest.tipos_motivo.length) {
				logger.error("El valor del indice para acceder al contenido del combo tipos_motivo es incorrecto: '"+indice+"'");
				return false;
			}
			else {
				row[1]=MotivosTest.tipos_motivo[indice-1];
	...
	public boolean sintacticAnalysis(String titulo, String subtitulo, String[] botones, String[] botonesCRUD,
		...
		logger.debug("Motivos: inicio análisis sintáctico");
